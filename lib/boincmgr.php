...
//      global $username;
        ...
        if(auth_is_admin($username)==FALSE) {
                ...
                $project_correct=db_query_to_variable("SELECT 1 FROM `projects` WHERE `uid`='$project_uid_escaped' AND `status` IN ('enabled','auto enabled')");
        }
...
function boincmgr_get_mag_per_project() {
        ...
        $whiltelisted_count=db_query_to_variable("SELECT count(*) FROM `projects` WHERE `status` IN ('enabled','auto enabled','stats only') AND `present_in_superblock`=1");
...
WHERE bphl.`project_uid`='$project_uid_escaped' AND bphl.`host_uid`='$host_uid_escaped'
    AND bp.`status` IN ('enabled','auto enabled','stats only') AND bp.`present_in_superblock`=1");
...
WHERE bphl.`project_uid`='$project_uid_escaped' AND bh.`username_uid`='$user_uid_escaped' AND bp.`status` IN ('enabled','auto enabled','stats only') AND bp.`present_in_superblock`=1");
        if($relative_contribution=="") $relative_contribution=0;
        return $relative_contribution;
}
...
WHERE bp.`uid`='$project_uid_escaped' AND bp.`status` IN ('enabled','auto enabled','stats only') AND bp.`present_in_superblock`=1");
        if($relative_contribution=="") $relative_contribution=0;
        return $relative_contribution;
}
...
function boincmgr_get_relative_contribution_host($host_uid) {
        ...
        $relative_contribution=db_query_to_variable("SELECT SUM(bphl.`expavg_credit`/bp.`team_expavg_credit`) FROM `project_hosts_last` AS bphl
LEFT OUTER JOIN `projects` AS bp ON bp.uid=bphl.project_uid
WHERE bphl.`host_uid`='$host_uid_escaped' AND bp.`status` IN ('enabled','auto enabled','stats only') AND bp.`present_in_superblock`=1");
        if($relative_contribution=="") $relative_contribution=0;
        return $relative_contribution;
}
...
LEFT OUTER JOIN `projects` AS bp ON bp.uid=bphl.project_uid
LEFT OUTER JOIN `hosts` AS bh ON bh.uid=bphl.host_uid
WHERE bh.`username_uid`='$user_uid_escaped' AND bp.`status` IN ('enabled','auto enabled','stats only') AND bp.`present_in_superblock`=1");
        if($relative_contribution=="") $relative_contribution=0;
        return $relative_contribution;
}
