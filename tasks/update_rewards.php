...
WHERE `project_uid`='$project_uid_escaped' AND `host_uid`='$host_uid_escaped' AND `host_id`='$host_id_escaped' AND `timestamp`<'$timestamp_escaped' ORDER BY `timestamp` DESC LIMIT 1");
	...
	if(count($prev_point)==0) {
		...
		db_query("UPDATE `project_host_stats` SET `interval`=0,`magnitude_unit`='$magnitude_unit_escaped',`grc_amount`=0,
`exchange_rate`='$rate_per_grc_escaped',`currency`='$currency_escaped',`currency_amount`='0',`is_payed_out`=1 WHERE `uid`='$uid_escaped'");
