...
$data = db_query_to_array("
        ...
        AND `timestamp` > DATE_SUB(CURRENT_DATE, INTERVAL 2 MONTH)
...
foreach($data as $row) {
        ...
        ");
        db_query("
                DELETE FROM `project_host_stats`
                WHERE `uid` <> '$min_uid'
                        AND `project_uid` = '$project_uid'
                        AND `host_uid` = '$host_uid'
                        AND `host_id` = '$host_id' AND
                        COALESCE(`currency`,'GRC') = '$currency'
                        AND DATE(`timestamp`) = '$day_timestamp'
        ");
        db_query("COMMIT");
//die();
}
