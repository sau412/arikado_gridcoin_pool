...
$host_uid_array=db_query_to_array("SELECT `uid` FROM `boincmgr_hosts`");
$index=1;
foreach($host_uid_array as $host_uid_row) {
        ...
        $project_uid_array=db_query_to_array("SELECT `uid` FROM `boincmgr_projects`");
...
$user_uid_array=db_query_to_array("SELECT `uid` FROM `boincmgr_users`");
$index=0;
